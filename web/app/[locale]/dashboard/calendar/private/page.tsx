    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-[24px] font-bold text-gray-900">{t("privateTitle")}</h1>
                    <p className="text-gray-500 text-[14px]">{t("privateSubtitle")}</p>
                </div>
                <Link href={`/${locale}/dashboard/calendar/new?scope=private`}>
                    <Button className="px-6! py-2! text-[14px]!"><Plus className={`h-4 w-4 ${locale === "en" ? "mr-2" : "ml-2"}`} />{t("addEvent")}</Button>
                </Link>
            </div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-semibold text-gray-900">
                                        {view === "month"
                                            ...
                                            : `${(() => { const start = new Date(current); start.setDate(current.getDate() - start.getDay()); return start; })().toLocaleDateString(locale, { month: "short", day: "numeric" })} – ${(() => { const start = new Date(current); start.setDate(current.getDate() - start.getDay() + 6); return start; })().toLocaleDateString(locale, { month: "short", day: "numeric", year: "numeric" })}`}
                                    </span>
                                </div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <a href="/api/events/ics?scope=private" className="text-xs px-3 py-2 rounded-md bg-brand-primary text-white hover:opacity-90 shadow-sm">{t("downloadICS") || "Download ICS"}</a>
                                </div>
                            </div>
                            {(() => {
                                ...
                                const weekStart = (() => { const dt = new Date(current); const day = dt.getDay(); const st = new Date(dt); st.setDate(dt.getDate() - day); st.setHours(0, 0, 0, 0); return st; })();
                                const weekDays = Array.from({ length: 7 }, (_, i) => { const d = new Date(weekStart); d.setDate(weekStart.getDate() + i); return d; });
                                const weekdayHeaders = Array.from({ length: 7 }, (_, i) => { const base = new Date(1970, 0, 4 + i); return base.toLocaleDateString(locale, { weekday: "short" }); });
                                return (
                                    view === "month" ? (
                                        <div className="grid grid-cols-7 gap-px bg-gray-200 rounded-md overflow-hidden">
                                            ...
                                            {weeks.flat().map((day, idx) => (
                                                <div key={idx} className="bg-white min-h-[90px] p-2">
                                                    {!isNaN(day.getTime()) && (
                                                        <div className="text-xs font-semibold text-gray-900">{day.getDate()}</div>
                                                    )}
                                                    <div className="mt-1 space-y-1">
                                                        {!isNaN(day.getTime()) && events.filter((ev) => {
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            {weekDays.map((day, idx) => (
                                                <div key={idx} className="bg-white min-h-[120px] rounded-md border border-gray-200" onDragOver={(e) => { e.preventDefault(); e.dataTransfer.dropEffect = "move"; }} onDrop={(e) => { const id = e.dataTransfer.getData("text/event-id") || e.dataTransfer.getData("text/plain"); if (id) moveEvent(id, day); }}>
