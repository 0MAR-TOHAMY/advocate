...
import { uploadCover, deleteFromCloudinary } from "@/lib/cloudinary";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user.coverUrl) {
      const publicId = user.coverUrl.split("/").slice(-2).join("/").split(".")[0];
      await deleteFromCloudinary(publicId);
    }

    // Upload new cover
    const result = await uploadCover(fileData, payload.userId);

    // Update user cover URL in database
    await db
      .update(users)
      .set({
