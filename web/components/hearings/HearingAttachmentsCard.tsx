...
import HearingAttachmentUploadDialog from "./HearingAttachmentUploadDialog";
import { Badge } from "@/components/ui/Badge";
import AlertModal from "@/components/ui/AlertModal";
import { cn } from "@/lib/utils";
import ModalButton from "@/components/ui/ModalButton";

interface HearingAttachmentsCardProps {
    hearingId: string;
}

export default function HearingAttachmentsCard({ hearingId }: HearingAttachmentsCardProps) {
    ...
    const fetchAttachments = async () => {
        try {
            setLoading(true);
            const res = await fetch(`/api/hearings/${hearingId}/attachments`);
            if (res.ok) {
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/hearings/${hearingId}/attachments?attachmentId=${id}`, {
                method: "DELETE",
            });
        ...
        <div className="bg-white p-8 rounded-[32px] border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)]">
            ...
            {loading ? (
                        ...
                        >
                            <div className={cn("absolute top-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity", isAr ? "left-0" : "right-0")}>
                                <div className="flex gap-1">
                                    <a
                                        href={file.fileUrl?.startsWith("http") ? file.fileUrl : `/api/documents/download?key=${encodeURIComponent(file.fileUrl)}&name=${encodeURIComponent(file.fileName)}`}
                                        target="_blank"
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className={cn("flex-1 min-w-0", isAr ? "pl-16" : "pr-16")}>
                                    <p className="font-black text-gray-900 truncate text-sm mb-1">{file.fileName}</p>
                ...
                <div className="text-center py-20 border-2 border-dashed border-gray-100 rounded-[24px] bg-gray-50/20 group">
                    ...
                    <p className="text-sm font-black text-gray-400 uppercase tracking-widest">{isAr ? "لا توجد مرفقات لهذه الجلسة" : "No attachments for this hearing"}</p>
                    <p className="text-[11px] font-bold text-gray-300 mt-2 max-w-[200px] mx-auto leading-relaxed">
                        {isAr ? "قم برفع المستندات والوثائق المتعلقة بالجلسة هنا" : "Upload documents and records related to this hearing here"}
                    </p>
                </div>
            )}

            <HearingAttachmentUploadDialog
                open={uploadOpen}
